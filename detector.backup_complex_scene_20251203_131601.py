...
class SAM3Detector:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Initialize processor with lower confidence threshold for better detection
        # FIX FINAL: Bajar aún más el threshold para detectar 30-50 objetos (production-ready)
        # Default is 0.5, but we use 0.01 to catch maximum objects including small/dark/occluded/overlapping ones
        # Lower threshold helps detect small objects, dark objects, partially occluded objects, and overlapping objects
        # Post-filtering in config.yaml will remove false positives
        self.processor = Sam3Processor(
            self.model, 
            device=actual_device,
            confidence_threshold=0.01  # FIX FINAL: REDUCED from 0.02 to 0.01 for 30-50 detections (production-ready)
        )
    ...
    def detect_objects(
        ...
        # Use text prompt if provided, otherwise use automatic detection
        if text_prompt:
            ...
            # Automatic detection: use "visual" prompt (SAM 3's internal dummy for visual-only detection)
            # "object" doesn't work well, but "visual" enables visual grounding without specific text
            logger.info("Using automatic detection (visual grounding)")
            output = self.processor.set_text_prompt(state=inference_state, prompt="visual")
        
        # Extract masks, boxes, and scores
        # SAM 3 returns: masks (Tensor), boxes (Tensor [x0, y0, x1, y1]), scores (Tensor)
        masks = output.get("masks")  # Tensor shape: [N, 1, H, W]
        boxes = output.get("boxes")  # Tensor shape: [N, 4] in format [x0, y0, x1, y1]
        scores = output.get("scores")  # Tensor shape: [N]
        
        if masks is None or boxes is None or scores is None:
            logger.warning(f"SAM 3 output incomplete. Keys: {list(output.keys())}")
            return []
        
        num_objects = len(boxes)
        logger.info(f"SAM 3 raw output: {num_objects} objects detected")
