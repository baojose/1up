...
class SAM3Detector:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # FIX ESCENAS COMPLEJAS: Bajar threshold a 0.005 para detectar 40-60 objetos (25-30 objetos reales)
        # Default is 0.5, but we use 0.005 to catch maximum objects including small/dark/occluded/overlapping ones
        ...
        self.processor = Sam3Processor(
            ...
            confidence_threshold=0.005  # FIX ESCENAS COMPLEJAS: REDUCED from 0.01 to 0.005 for 40-60 detections (25-30 real objects)
        )
    ...
    def detect_objects(
        ...
        # FIX ESCENAS COMPLEJAS: Para escenas con 25-30 objetos, usar múltiples prompts
        # Si no se especifica text_prompt, ejecutar múltiples detecciones y combinar resultados
        all_detections = []
        ...
        if text_prompt:
            ...
            # Detección múltiple para escenas complejas (25-30 objetos)
            # Usar diferentes prompts para detectar diferentes categorías de objetos
            prompts = [
                "visual",  # Detección general (objetos visuales)
                "container",  # Contenedores, frascos, botellas
                "object",  # Objetos genéricos
            ]
            
            logger.info(f"Using multi-prompt detection for complex scenes ({len(prompts)} prompts)")
