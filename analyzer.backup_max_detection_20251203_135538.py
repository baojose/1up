...
{{
  "validated_objects": [
    {{"n": 1, "useful": "yes", "name": "Pesa de entrenamiento negra", "category": "sports", "condition": "good", "description": "Pesa de entrenamiento negra con forma redonda", "estimated_value": "20-40€", "bbox": [x, y, w, h], "bbox_confidence": "high"}},
    {{"n": 2, "useful": "yes", "name": "Silla de oficina", "category": "furniture", "condition": "fair", "description": "Silla con patas oscuras y respaldo", "estimated_value": "30-60€", "bbox": [x, y, w, h], "bbox_confidence": "high"}},
    {{"n": 3, "useful": "yes", "name": "Ropa doblada (camisetas y pantalones)", "category": "clothing", "condition": "good", "description": "Pila de ropa doblada con varias prendas", "estimated_value": "5-15€", "bbox": [x, y, w, h], "bbox_confidence": "medium"}},
    {{"n": 4, "useful": "no", "reason": "Fragmento de fondo"}},
    {{"n": 5, "useful": "yes", "name": "Objeto superpuesto detectado", "category": "other", "condition": "good", "description": "Objeto adicional encontrado en región superpuesta", "estimated_value": "10-20€", "bbox": [x, y, w, h], "bbox_confidence": "low"}},
    ...
  ],
  "missing_objects": [
...
IMPORTANTE - AGRUPACIÓN DE OBJETOS SIMILARES (CRÍTICO):
- Si ves MÚLTIPLES objetos IDÉNTICOS o muy similares superpuestos (ejemplo: 6 frascos de especias en un especiero), 
  NO los listes uno por uno como objetos separados
- En su lugar, crea UN SOLO objeto con descripción grupal que mencione la cantidad
- Ejemplo CORRECTO: "Especiero con 6 frascos de especias variadas" (NO "Frasco 1, Frasco 2, Frasco 3...")
- Ejemplo CORRECTO: "Set de 3 cuchillos en soporte" (NO "Cuchillo 1, Cuchillo 2, Cuchillo 3...")
- Ejemplo CORRECTO: "Grupo de 4 botellas de aceite" (NO "Botella 1, Botella 2...")
- Usa cantidad en el nombre: "Set de X", "Grupo de X", "Colección de X", "Especiero con X frascos"
- Si los objetos están muy juntos y son del mismo tipo, considéralos un GRUPO, no objetos individuales
- REGLA: Si hay 3+ objetos similares superpuestos → crear 1 objeto grupal

IMPORTANTE - GENERACIÓN DE BBOXES (CRÍTICO):
- Si el objeto es CLARAMENTE visible: genera bbox preciso con coordenadas exactas [x, y, w, h]
- Si el objeto está PARCIALMENTE oculto: genera bbox del área visible (no del objeto completo)
- Si el objeto está MUY superpuesto: genera bbox aproximado del área principal que se ve
- Si tienes CUALQUIER duda sobre coordenadas: genera un bbox aproximado del centro del objeto
- NUNCA omitas el campo "bbox" - SIEMPRE debe estar presente, aunque sea una estimación
- Si realmente no puedes ver el objeto: marca "useful": "no" en lugar de omitir el bbox
- Para objetos grupales: el bbox debe cubrir TODO el grupo (ej: todo el especiero, no solo un frasco)

Formato del bbox: [x, y, width, height] donde:
- x, y = coordenadas de la esquina superior izquierda (en píxeles)
- width, height = ancho y alto del objeto (en píxeles)
- Origen (0,0) está en la esquina superior izquierda de la imagen

IMPORTANTE:
- Responde para TODOS los {num_objects} objetos detectados
- **USA EL TAMAÑO PARA IDENTIFICAR CORRECTAMENTE** cada objeto
- **CRÍTICO**: Si useful="yes", DEBES incluir "bbox" con las coordenadas [x, y, w, h] (SIEMPRE, aunque sea aproximado)
- El bbox debe corresponder al objeto real que identificaste, no necesariamente al bbox original si no coincide
- Añade también "bbox_confidence": "high|medium|low" para indicar qué tan seguro estás del bbox
- missing_objects puede estar vacío [] si no hay objetos faltantes
- NO incluyas markdown (```json)
- Si useful="yes" DEBES incluir todos los campos (name, category, condition, description, estimated_value, bbox, bbox_confidence)"""
        else:  # English
            return f"""Analyze this recycling center scene.
...
{{
  "validated_objects": [
    {{"n": 1, "useful": "yes", "name": "...", "category": "...", "condition": "...", "description": "...", "estimated_value": "...", "bbox": [x, y, w, h], "bbox_confidence": "high|medium|low"}},
...
IMPORTANT - GROUPING SIMILAR OBJECTS (CRITICAL):
- If you see MULTIPLE IDENTICAL or very similar overlapping objects (example: 6 spice jars in a spice rack),
  DO NOT list them one by one as separate objects
- Instead, create A SINGLE object with a group description that mentions the quantity
- CORRECT example: "Spice rack with 6 spice jars" (NOT "Jar 1, Jar 2, Jar 3...")
- CORRECT example: "Set of 3 knives in holder" (NOT "Knife 1, Knife 2, Knife 3...")
- CORRECT example: "Group of 4 oil bottles" (NOT "Bottle 1, Bottle 2...")
- Use quantity in the name: "Set of X", "Group of X", "Collection of X", "Spice rack with X jars"
- If objects are very close together and of the same type, consider them a GROUP, not individual objects
- RULE: If there are 3+ similar overlapping objects → create 1 group object

IMPORTANT - BBOX GENERATION (CRITICAL):
- If the object is CLEARLY visible: generate precise bbox with exact coordinates [x, y, w, h]
- If the object is PARTIALLY occluded: generate bbox of the visible area (not the complete object)
- If the object is VERY overlapped: generate approximate bbox of the main visible area
- If you have ANY doubt about coordinates: generate an approximate bbox of the object center
- NEVER omit the "bbox" field - it MUST always be present, even if it's an estimation
- If you really cannot see the object: mark "useful": "no" instead of omitting the bbox
- For group objects: the bbox should cover the ENTIRE group (e.g., the whole spice rack, not just one jar)

Bbox format: [x, y, width, height] where:
- x, y = coordinates of top-left corner (in pixels)
- width, height = object width and height (in pixels)
- Origin (0,0) is at the top-left corner of the image

IMPORTANT:
- Respond for ALL {num_objects} detected objects
- **CRITICAL**: If useful="yes", you MUST include "bbox" with coordinates [x, y, w, h] (ALWAYS, even if approximate)
- The bbox should correspond to the actual object you identified, not necessarily the original bbox if it doesn't match
- Add also "bbox_confidence": "high|medium|low" to indicate how confident you are about the bbox
